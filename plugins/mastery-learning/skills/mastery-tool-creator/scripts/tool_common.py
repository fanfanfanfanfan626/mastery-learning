...
def _verification_report_matches(catalog_path: Path, entry: dict[str, Any], tool_id: str, digest: str) -> bool:
    ...
    if not isinstance(report_sha256, str) or hashlib.sha256(report_bytes).hexdigest() != report_sha256:
        return False
    required_keys = {
        "schema_version", "tool_id", "tool_version", "verified_at", "sandboxed_by", "review_notes",
        "check", "inspection", "concept_registration", "manifest_sha256", "tool_snapshot", "warning",
    }
    if (
        ...
        or report.get("sandboxed_by") != "codex-workspace-sandbox"
