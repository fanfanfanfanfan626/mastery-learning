#!/usr/bin/env python3
"""Archive Codex-observed check/render results; never execute generated code."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Finalize a teaching tool from externally observed sandbox/render results")
    parser.add_argument("tool_dir", type=Path)
    parser.add_argument("--sandboxed-by", required=True, choices=["codex-workspace-sandbox"])
    ...
    if request:
        if args.observed_exit_code is None or args.observed_output_file is None:
            raise SystemExit("A declared check requires --observed-exit-code and --observed-output-file from the Codex sandbox run")
    ...
    require_current_validation(mastery_root / "tool-catalog.json", manifest["id"], root, snapshot["sha256"], SCHEMA_VERSION)
    manifest_sha256 = next(item["sha256"] for item in snapshot["files"] if item["path"] == "tool.json")
    report = {
        "schema_version": 3, "tool_id": manifest["id"], "tool_version": manifest["version"],
        "verified_at": timestamp(), "sandboxed_by": args.sandboxed_by, "review_notes": args.review_notes.strip(),
        "check": observed,
        "inspection": {
        ...
        "warning": "This report records Codex-observed results; it is not a portable operating-system security sandbox attestation.",
    }
    ...
    }, SCHEMA_VERSION, remove_keys=("stale_at", "stale_reason", "rejected_at", "validation_errors"))
