...
```json
{
  ...
  "launch": "From this tool directory run `python -m http.server 8000 --bind 127.0.0.1`, inspect `http://127.0.0.1:8000/index.html`, then stop the server with Ctrl+C.",
...
`code_lab`, `visual_lab`, `simulation_3d`, `blackboard`, `notebook`, `quiz`, `slide_deck`, `document`, `project_lab`.
...
Set `inspection.required` for visual labs, simulations, slides, and documents. Do not add a self-attested `completed` field. Put the intended inspection scope in the manifest; after rendering, pass actual states/pages, accessibility fallback, and viewport observations to `finalize_tool.py`, with the separate structured `--inspection-result passed` gate. A failed or missing required inspection cannot finalize. Serve HTML with `python -m http.server <port> --bind 127.0.0.1` and inspect the loopback URL; do not rely on `file://`. Every HTML page must carry the exact local-only CSP generated by `tool_scaffold.py`; do not weaken or remove a directive. All executable/resource references must resolve inside the snapshotted tool directory. Put remote source URLs in `sources` or passive HTTPS anchors with `rel="noopener noreferrer"`, never in scripts, styles, frames, forms, workers, or dynamic loaders. Browser-inspected visual and 3D labs use a local HTML fallback linked from the entrypoint; static validation rejects missing local HTML resources and an entrypoint that does not directly link the declared fallback. Text-native tools may use Markdown. A complete slide deck must point to a real `.pptx`; a complete document must point to a real `.docx` or `.pdf`.
