...
def validate_manifest(manifest: Any, errors: list[str]) -> None:
    ...
    if not isinstance(inspection, dict):
        ...
        if not isinstance(inspection.get("notes"), str) or not inspection.get("notes", "").strip():
            errors.append("inspection.notes must describe what the active AI host must inspect")
...
def declared_check(root: Path, manifest: dict[str, Any], errors: list[str]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "instruction": "Run this command only through an isolated or explicitly learner-authorized host execution boundary, with secrets removed and the declared no-cache environment applied; then pass observed results and host provenance to finalize_tool.py.",
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Statically validate a Mastery Tutor teaching tool without executing it")
