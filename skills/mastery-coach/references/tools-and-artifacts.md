...
- In **pair mode**, let the learner choose architecture and invariants; the AI may perform repetitive edits.
...
Never run untrusted code outside an isolated or explicitly learner-authorized environment. Static
manifest validation does not make code safe: execute the check in a separate host tool call with
secrets removed, record the observer and execution boundary, then archive only observed exit/output
through the Tool Creator finalization gate. Do not expose secrets in examples, logs, fixtures, or
screenshots.
